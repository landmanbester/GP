...
def func(x,y):
    return 5*np.sin(np.sqrt(2.0*x**2 + 5.0*y**2))
...
if __name__=="__main__":
    ...
    z = func(x[0,:],x[1,:]) + 0.01*np.random.randn(N)    
    
    GP = Class2DGP(x, xp)
    GP.set_abs_diff()
    
    # Set theta0
    theta0 = np.array([10.0,2.0,0.05])
    
    #GP.logp_and_gradlogp(theta0,z)
    
    
    zp, theta = GP.EvalGP(theta0,z)
    print theta
    zp = zp.reshape((Np,Np))
